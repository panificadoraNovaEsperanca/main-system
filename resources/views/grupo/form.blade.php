...
@section('content')
    <form enctype="multipart/form-data"
        action="{{ isset($grupo) ? route('grupo.update', $grupo->id) : route('grupo.store') }}" method="POST">
        @csrf
        @if (isset($grupo))
            @method('PUT')
        @endif
        <div class="row">
            <div class="col-12">
                ...
                <input value="{{ isset($grupo) ? $grupo->nome : '' }}" class="form-control" name="nome" id="nome">
                @error('nome')
                ...
                <div class="col-3">
                    <div class="form-check">
                        <input class="form-check-input"
                            @if (isset($permissoesSelecionadas)) {{ in_array($permissao->id, $permissoesSelecionadas) ? 'checked' : '' }} @endif
